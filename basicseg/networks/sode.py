...
MAX_NUM_STEPS = 5 # Maximum number of steps for ODE solver
...
class ODEBlock(nn.Module):
    ...
    """
    def __init__(self, odefunc, is_conv=False, tol=1e-3 , adjoint=True):
    ...
    def forward(self, x, eval_times=None):
        ...
        if eval_times is None:
            ...
            integration_time = torch.from_numpy(np.linspace(0, 1, 5)).cuda()
        else:
        ...
        if self.adjoint:
            out = odeint_adjoint(self.odefunc, x_aug, integration_time,
                                 rtol=self.tol, atol=self.tol, method='rk4')
        else:
...
class ODENet(nn.Module):
    ...
    adjoint : bool
        ...
        backpropagates directly through operations of ODE solver.
    """
    def __init__(self, data_dim=192, hidden_dim=100,
                 augment_dim=0, time_dependent=False, non_linearity='relu',
                 tol=1e-3, adjoint=True):
        ...
        self.odeblock = ODEBlock(odefunc, tol=tol, adjoint=adjoint)
